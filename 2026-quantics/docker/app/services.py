...
async def use_leqo_db() -> AsyncGenerator[AsyncEngine]:
    ...
    load_dotenv()
    print(os.environ["SQLALCHEMY_DRIVER"])
    print(os.environ["LEQO_POSTGRES_USER"])
    print(os.environ["LEQO_POSTGRES_PASSWORD"])
    print(os.environ["LEQO_POSTGRES_HOST"])
    print(int(os.environ["LEQO_POSTGRES_PORT"]))
    print(os.environ["LEQO_POSTGRES_DB"])

    url = DataBaseURL.create(
        drivername=os.environ["SQLALCHEMY_DRIVER"],
        username=os.environ["LEQO_POSTGRES_USER"],
        password=os.environ["LEQO_POSTGRES_PASSWORD"],
        host=os.environ["LEQO_POSTGRES_HOST"],
        port=int(os.environ["LEQO_POSTGRES_PORT"]),
        database=os.environ["LEQO_POSTGRES_DB"],
    )
